...
const ExcelExport = {
    ...
    generateReport: (expenses, aggregation, summary) => {
        const wb = XLSX.utils.book_new();

        // Sheet 1: Summary by Category
        const summaryData = ExcelExport.createSummarySheet(summary);
        const summaryWs = XLSX.utils.aoa_to_sheet(summaryData);
        ExcelExport.styleSheet(summaryWs, summaryData);
        XLSX.utils.book_append_sheet(wb, summaryWs, "Summary");

        // Sheet 2: Monthly Breakdown
        const monthlyData = ExcelExport.createMonthlySheet(aggregation, summary);
        const monthlyWs = XLSX.utils.aoa_to_sheet(monthlyData);
        ExcelExport.styleSheet(monthlyWs, monthlyData);
        XLSX.utils.book_append_sheet(wb, monthlyWs, "Monthly Breakdown");

        // Sheet 3: All Transactions
        const transactionsData = ExcelExport.createTransactionsSheet(expenses);
        const transactionsWs = XLSX.utils.aoa_to_sheet(transactionsData);
        ExcelExport.styleSheet(transactionsWs, transactionsData);
        ...
        const categoryDetailData = ExcelExport.createCategoryDetailSheet(expenses);
        const categoryDetailWs = XLSX.utils.aoa_to_sheet(categoryDetailData);
        ExcelExport.styleSheet(categoryDetailWs, categoryDetailData);
